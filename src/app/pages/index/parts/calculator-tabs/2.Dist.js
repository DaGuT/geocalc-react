...
import './dist-assets/dist.css';
import templ from "./dist-assets/template.txt"

let params = {
  'displayName': 'Дистанционная',
  'blockID': 'dist-zadacha',
  'icon': ''
};
...
function later() {
  ...
  $('[name="' + name + '"]').click(function() { //this is adding even listener for click event
    ...
    if ($('.' + params.blockID + '[name=' + name + ']:checked').length) { //if we have any chechbox(radiobutton) selected then we display block
      ...
      if (document.querySelector("#" + params.blockID + "inputGroupFile01").value!="") $('#step3-' + params.blockID).show(400);
    }
